...
After navigating to `url/login` and logging in with credentials `wiener - peter`, we are taken to `url/my-account?id=wiener`, and are presented an option to upload an image as our avatar. Observing the intercepted HTTP POST request reveals the file is transmitted as `multipart/form-data`. The boundary contains the `Content-Type: image/jpeg` header. The server responds with a `200 OK`.
...
3. On `url/my-account?id=wiener`, right click on your "avatar" and select "Open Image in New Tab". You'll be taken to `url/files/avatars/shell.php`, with a shell to execute commands. The `php` script has been executed as code.
